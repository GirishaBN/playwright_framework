...
- Name: Search.spec.ts >> verify user is able to land on the product page test
- Location: tests\Search.spec.ts:16:1
...
```ts
  ...
  7  |     await loginPage.doLogin(config.username,config.password);
  8  | });
  9  | 
  10 | test('verify search results count test',async({homePage,searchResultPage})=>{
  11 |     await homePage.searchProduct("macbook");
  12 |     let searchCount=await searchResultPage.getProductSearchResultCount();
  13 |     expect(searchCount).toBe(3)
  14 | })
  15 | 
  16 | test('verify user is able to land on the product page test',async({page,homePage,searchResultPage})=>{
  17 |     await homePage.searchProduct("macbook");
  18 |     await searchResultPage.selectProduct('MacBook Pro');
> 19 |     expect(await page.title()).toEqual('MacBook Pro')
     |                                ^ Error: expect(received).toEqual(expected) // deep equality
  20 | })
```