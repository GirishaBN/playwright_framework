...
- Location: tests\api\user.api.spe.spec.ts:39:1

# Error details

```
TypeError: apiHelper.delet is not a function
```
...
```ts
  ...
  35 | expect(response.body.name).toBe(userUpdatedData.name);
  36 | expect(response.body.status).toBe(userUpdatedData.status);
  37 | })
  38 | 
  39 | test('Delete API: delete user',async({apiHelper})=>{
> 40 | let response=await apiHelper.delet(`/public/v2/users/${userid}`,AUTH_HEADER);
     |                              ^ TypeError: apiHelper.delet is not a function
