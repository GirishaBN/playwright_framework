...
> 35 | expect(response.status).toBe(200);
     |                         ^ Error: expect(received).toBe(expected) // Object.is equality
  36 | expect(response.body.name).toBe(userUpdatedData.name);
  37 | expect(response.body.status).toBe(userUpdatedData.status);
  38 | })
  39 | 
  40 | test('Delete API: delete user',async({apiHelper})=>{
  41 | let response=await apiHelper.delete(`/public/v2/users/${userid}`,AUTH_HEADER);
