...
> 18 |     expect(response.status).toBe(201);
  ...
  30 |     const userid = userResponse.id;
  31 |     let getResponse = await apiHelper.get(`/public/v2/users/${userid}`, AUTH_HEADER);
  32 |     expect(getResponse.status).toBe(200);
  33 |     expect(getResponse.statusText).toBe('OK');
  34 | })
  35 | 
  36 | test('PUT API: update user', async ({ apiHelper }) => {
  37 |     let userresponse = await createUser(apiHelper);
  38 |     const userid = userresponse.id;
  39 |     let userUpdatedData = {
  40 |         name: `mahadevamma update ${Date.now()}`,
  41 |         gender: "female",
  42 |         status: "inactive"
  43 |     }
  ...
  57 |     const userid = userresponse.id;
  ...
  61 |     expect(getResponse.status).toBe(404);
  62 |     expect(getResponse.statusText).toBe('Not Found');
  63 | })
  64 | 
```